document.addEventListener('DOMContentLoaded', () => {
  ...
  toggleButton.addEventListener('click', async () => {
    ...
    const isRunning = toggleButton.textContent === 'Démarrer';
    toggleButton.textContent = isRunning ? 'Arrêter' : 'Démarrer';
  ...
  exportButton.addEventListener('click', () => {
    chrome.storage.local.get('allProfiles', (data) => {
      ...
      profiles.forEach(profile => {
        ...
        totalValue += Number(profile.value) || 0;
        totalCases += Number(profile.cases) || 0;
      });

      csv += `Total,${totalValue},${totalCases}\n`;
